...
public class Client {
    ...
    public long[] sendRequest(int mode, int n, int m, int p)
            throws IOException {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int mode = 1; mode <= 2; mode++) {
            System.out.println();
            if (mode == 1) {
                System.out.println("=== Mode 1: матриці генерує СЕРВЕР ===");
            } else {
                System.out.println("=== Mode 2: матриці генерує КЛІЄНТ ===");
            }
            System.out.printf(
                    "%-8s  %-16s  %-16s  %-16s%n",
                    "Розмір", "Відправка (мс)", "Отримання (мс)", "Загалом (мс)"
            );
